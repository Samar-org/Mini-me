...
class AirtableImageDownloader:
    ...
    def process_record(self, record):
        ...
        sku_field = os.getenv('AIRTABLE_HIBID_LOT_FIELD', 'Hibid Lot No')
        inventory_no = fields.get(sku_field, f'unknown_{record_id}')
        inventory_no = "".join(c for c in str(inventory_no) if c.isalnum() or c in ('-', '_'))

        # Get and sanitize auction code
        auction_code = fields.get("Auction-code", "001")
        auction_code_clean = "".join(c for c in str(auction_code) if c.isalnum() or c in ('-', '_'))
        auction_folder = os.path.join(self.output_dir, f"Auction-{auction_code_clean}")
        os.makedirs(auction_folder, exist_ok=True)

        logger.info(f"Processing item: {inventory_no} | Auction: {auction_code_clean} (Record ID: {record_id[:8]}...)")
        image_counter = 1
        ...
        image_fields = [featured_photo_field, item_photos_field, inspection_photos_field]
                ...
                for attachment in attachments:
                    if 'url' in attachment:
                        ...
                        if image:
                            ...
                            processed_image.save(filepath, 'JPEG', quality=90, optimize=True)
    ...
    def save_download_report(self):
        ...
        report_path = os.path.join(
            self.output_dir, f"download_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")

        with open(report_path, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['inventory_no', 'filename', 'field_name', 'auction_code',
                          'view_name', 'original_url', 'timestamp']
            ...
            for page in self.table.iterate(page_size=100, view=view_name):
                for record in page:
                    try:
                        ...
                        failed_downloads += 1
                        logger.error(f"Failed to process record {record.get('id', 'unknown')}: {str(e)}")
                        continue
