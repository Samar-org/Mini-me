...
def upload_image(image_url: str, product_name: str, index: int) -> Optional[int]:
    """Downloads, optimizes, uploads, and sets metadata for a single image."""
    # Step 1: Download and optimize the image
    ...
    # Step 2: Upload the image file to WordPress
    headers = {
    ...
    # Brief pause to allow the server to process the uploaded file
    time.sleep(1)

    # Step 3: Update the media item with SEO metadata
    logger.info(f"Updating SEO metadata for media ID: {media_id}...")

    # Define SEO metadata payload
    alt_text = (
        f"{product_name} - Main product photo"
        if index == 0
        else f"{product_name} - Photo {index + 1}"
    )
    metadata_payload = {
        "alt_text": alt_text,
    ...
    # Send POST request to update the metadata (PATCH is also valid)
    update_response = requests.post(
        metadata_url, headers=metadata_headers, json=metadata_payload
    )

    if update_response.status_code == 200:
        logger.info(f"✅ Successfully updated SEO metadata for image {media_id}.")
    else:
        logger.warning(
            f"Image uploaded but metadata update failed for {media_id}: {update_response.status_code} - {update_response.text}"
        )
