"""
Rank Math 100% SEO Field Generator for Airtable using OpenAI (GPT-5 Thinking)
-----------------------------------------------------------------------------
...
def sanitize_output(data: Dict[str, Any], fallback_slug_seed: str) -> Dict[str, Any]:
    ...
    if isinstance(alts, list):
        alts = "\n".join(a.strip() for a in alts if a.strip())
...
SYSTEM_PROMPT = (
    "You are GPT-5 Thinking, an elite ecommerce SEO copywriter and Rank Math expert. "
...
USER_INSTRUCTIONS = (
    ...
    "ecommerce_friendly_name, product_seo_title, meta_title, meta_description, focus_keyword, product_slug, image_alt_text.\n"
    "Rules: focus keyword FIRST in SEO title (50–60 chars) with one power word; meta title 50–60 chars; meta description 150–160 chars with the focus keyword once and a soft CTA; slug <=60 chars, hyphenated, includes the focus keyword; image_alt_text is an array of 4–6 strings including the focus keyword with unique angles.\n"
...
# Airtable I/O (compatible across pyairtable versions)
...
def fetch_products(table) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for chunk in iterator:
            # Some versions yield individual dict records; others yield page lists.
        ...
        # Older versions: fall back to .all
...
def process_record(table, rec: Dict[str, Any]) -> None:
    # Normalize when a page list slipped through accidentally
