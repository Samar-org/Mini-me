...
• Enforces: Do NOT mention “free delivery” or “same day delivery” in meta description or product tags.
...
Run
===
python v3-rankmath_seo_generator.py
"""
...
def clean_text(text: str) -> str:
    """Remove banned marketing phrases from text."""
    banned = [
        "free delivery",
        "same day delivery",
        "same-day delivery",
        # Optional extra safety (variants)
        "free shipping",
        "same day shipping",
        "same-day shipping",
    ]
    ...
    # Collapse double spaces and clean leftover punctuation
    t = re.sub(r"\s{2,}", " ", t)
    t = re.sub(r"\s+([,.;:!?])", r"\1", t)
    return t.strip()
...
def sanitize_output(data: Dict[str, Any], fallback_slug_seed: str) -> Dict[str, Any]:
    ...
    # Enforce: no "free delivery" / "same day delivery" in meta description
    out['Meta Description'] = clean_text(coalesce(data.get('meta_description')))
    ...
    # Product Tags sanitized (remove banned phrases)
    out['Product Tags'] = clean_text(coalesce(data.get('product_tags')))
    return out
...
SYSTEM_PROMPT = (
    ...
    "Critically IMPORTANT: Do not include the phrases 'free delivery' or 'same day delivery' (or variants like 'same-day delivery') "
    "in the meta description or product tags."
)
...
USER_INSTRUCTIONS = (
    ...
    "Compliance: Do NOT use the phrases 'free delivery' or 'same day delivery' (or their variants) "
    "in meta_description or product_tags.\n"
