...
class AirtableProductScraper:
    ...
    def extract_product_data_with_gemini(self, product_url: str, inspection_photo: str = None) -> Dict[str, Any]:
        ...
        If URL fails to provide information, use inspection photo as primary source.
        ...
        try:
            ...
            if webpage_content and len(webpage_content.strip()) > 500:  # Minimum content threshold
                logger.info("Attempting extraction from webpage content...")
                
                # Prepare the prompt for URL-based extraction
                url_prompt = f"""
                ...
                try:
                    # Generate response using Gemini for URL content
                    ...
                    # Check if extraction was successful (has meaningful product name)
                    ...
                    if (product_name and 
                        ...
                        # If we have inspection photo, use it to enhance/verify the data
                        if inspection_photo:
            ...
            # If URL extraction failed and we have inspection photo, use photo as primary source
            ...
            # If no inspection photo available, return error
    ...
    def enhance_with_inspection_photo(self, webpage_data: Dict[str, Any], inspection_photo: str, product_url: str) -> Dict[str, Any]:
        """
        Enhance webpage extraction data using inspection photo for verification and additional details.
        
        Args:
            webpage_data (Dict): Data extracted from webpage
            inspection_photo (str): Path or URL to inspection photo
        ...
        try:
            ...
            4. Add any missing details you can see (dimensions, weight if visible on packaging)
            5. Correct any inconsistencies between webpage data and image
            6. If the image shows a completely different product, prioritize the image information
            
            Return the enhanced data in the same JSON format:
            {{
                "Product Name": "verified/enhanced product name",
                "Description": "enhanced description with visual details",
                "Retail Price (CAD)": "price from webpage (keep original if not visible in image)",
                "Weight": "weight information (from image or webpage)",
                "Dimension": "dimensions (from image or webpage)",
                "Brand": "verified brand name",
                "Good product images": ["original image URLs from webpage"]
            }}
    ...
    def extract_from_inspection_photo(self, inspection_photo: str, product_url: str) -> Dict[str, Any]:
        ...
        Args:
            inspection_photo (str): Path or URL to inspection photo
            product_url (str): Original product URL (for context)
            
        Returns:
            Dict: Extracted product data from photo
        """
        try:
            ...
            The original product URL ({product_url}) did not provide sufficient information, so please rely primarily on what you can see in this image.
            ...
            {{
                ...
                "Good product images": ["Note: Extracted from inspection photo - original URL: {product_url}"]
            }}
            ...
            6. If information is not visible in the image, use "Not found" or make reasonable inferences
            7. Be as detailed and accurate as possible based on visual inspection
            
            Original URL (failed): {product_url}
            """
            
            # Generate response from photo
