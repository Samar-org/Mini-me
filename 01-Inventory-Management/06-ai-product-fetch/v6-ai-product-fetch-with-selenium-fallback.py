...
Extract product information using Gemini AI from URL and inspection photo.
        ...
        try:
            ...
            if not webpage_content or len(webpage_content.strip()) < 500:
                logger.warning("[Fallback] Using Selenium for content extraction...")
    ...
    def fetch_webpage_with_selenium(self, url: str) -> str:
        """Use Selenium to fetch page source for JavaScript-heavy pages like Amazon."""
        try:
            options = Options()
            ...
            driver.quit()
            logger.info(f"[Selenium] Successfully fetched page with browser for: {url}")
            return page_source
        except Exception as e:
            logger.warning(f"[Selenium] Failed to fetch page via Selenium: {e}")
            return ""
