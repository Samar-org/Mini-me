...
AIRTABLE_TABLE_NAME = "Product Catalogue"  # Changed from "Items-Bid4more"
AIRTABLE_VIEW_NAME = "AI-Product-Fetch"    # Changed from "Fetch-Program-View"

# Updated field mappings for Product Catalogue table
FIELD_MAPPINGS = {
    "url": "Product URL",
    "product_code": "Product Code",  # Changed from "4more-Product-Code"
    "name": "Product Name",
...
class AmazonScraper(BaseScraper):
    ...
    def scrape(self, url: str) -> ProductData:
        ...
        try:
            ...
            product.dimensions = details.get(
                "dimensions", "Product Dimensions Not Found"
            )
    ...
    def _extract_name(self, soup: BeautifulSoup) -> str:
        ...
        for selector in fallback_selectors:
            ...
            if element:
                ...
                if name and len(name) > 5:
                    logger.info(
                        f"Found product name using fallback selector: {selector}"
                    )
                    return name
            ...
            if price_el:
                ...
                if price_text:
                    ...
                    if price_match:
                        ...
                        if price_text.startswith("C$") or ".ca/" in soup.find(
...
class WalmartScraper(BaseScraper):
    ...
    def _extract_from_html(
        self, soup: BeautifulSoup, product: ProductData
    ) -> ProductData:
...
def process_airtable_records():
    ...
    logger.info("=== 🚀 Starting Enhanced Multi-Site Scraper for Product Catalogue ===")
    ...
    # Updated filter for Product Catalogue - looking for records with Status='Entered'
    ...
    for i, record in enumerate(records_to_process, 1):
        ...
        product_code = record["fields"].get(FIELD_MAPPINGS["product_code"])  # Changed from "4more-Product-Code"

        logger.info(
            f"\n--- Processing Record {i}/{len(records_to_process)} (ID: {record_id}) ---"
        )
        logger.debug(f"Product URL: {product_url}")
        logger.debug(f"Product Code: {product_code}")

        # Check if product code is already populated - skip scraping if so
        if product_code and str(product_code).strip():
            logger.info(
                f"✓ Product code '{product_code}' found in 'Product Code' field for record {record_id} - skipping scraping"
            )
            try:
                table.update(
                    record_id,
                    {
                        ...
                        FIELD_MAPPINGS[
                            "scraping_status"
                        ]: f"Skipped (Already linked to product code: {product_code})",
                    },
                )
                logger.info(
