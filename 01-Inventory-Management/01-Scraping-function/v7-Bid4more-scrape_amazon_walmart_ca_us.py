...
try:
    ...
    logger.error(
        "❌ pyairtable library not found. Please install it: pip install pyairtable"
    )
    Api = None
# endregion
...
class DataValidator:
    ...
    def validate_images(images: List[str]) -> bool:
        return images and all(
            isinstance(img, str) and img.startswith("http") for img in images
        )
    ...
    def validate_product(cls, product: ProductData) -> ProductData:
        ...
        if missing:
            ...
            logger.warning(
                f"Product {product.asin or product.url}: Missing fields - {', '.join(missing)}"
            )
        else:
            product.status = "Scraped"
            logger.info(
                f"Product {product.asin or product.url}: All critical fields validated"
            )
        return product
# endregion
    ...
    def get_page_content(self, url: str) -> Optional[BeautifulSoup]:
        ...
        try:
            ...
            response = self.session.get(
                url, headers=self.get_headers(), timeout=30, allow_redirects=True
            )
...
class AmazonScraper(BaseScraper):
    ...
    def scrape(self, url: str) -> ProductData:
        ...
        if any(domain in url for domain in ["amzn.to", "a.co"]):
            try:
                response = self.session.head(
                    url, allow_redirects=True, timeout=15, headers=self.get_headers()
                )
                url = response.url
                ...
                return ProductData(
                    ...
                    source="Amazon",
                    status="Failed",
                    error_details=f"URL resolution failed: {e}",
                )
        ...
        if not soup:
            return ProductData(
                ...
                source="Amazon",
                status="Failed",
                error_details="Failed to fetch page content",
            )

        # Check for various blocking patterns
        page_text_lower = soup.get_text().lower()
        if any(block_pattern in page_text_lower for block_pattern in [
            "robot or human",
            ...
            logger.warning(f"Blocked by Amazon anti-bot measures for URL: {url}")
            return ProductData(
                url=url,
                source="Amazon",
                status="Blocked",
                error_details="Blocked by Amazon (CAPTCHA/Robot Check)",
            )
        ...
        try:
            ...
            return ProductData(
                ...
                source="Amazon",
                status="Failed",
                error_details=f"Extraction error: {e}",
            )
    ...
    def _extract_name(self, soup: BeautifulSoup) -> str:
        ...
        # Extended fallback selectors for various Amazon layouts
        fallback_selectors = [
        ...
        for selector in fallback_selectors:
            ...
            if element:
                ...
                if name and len(name) > 3 and not name.lower().startswith('robot'):
        ...
        for attr_name, attr_value in meta_tags:
            ...
            if meta and meta.get("content"):
                ...
                # Clean up common Amazon title patterns
                if " : " in content:
                    content = content.split(" : ")[0]
                if " - " in content:
                    content = content.split(" - ")[0]
                if len(content) > 3:
                    logger.info(f"Found product name from meta tag: {attr_value}")
                    return content

        # Last resort: try to get title tag
        title_tag = soup.find("title")
        if title_tag:
            title_text = title_tag.get_text(strip=True)
            # Clean Amazon/Walmart suffixes
            for suffix in [" - Amazon.com", " - Amazon.ca", " : Amazon", " | Amazon", " - Walmart.com", " - Walmart.ca"]:
                if suffix in title_text:
                    title_text = title_text.replace(suffix, "")
            if len(title_text) > 3 and not title_text.lower().startswith('robot'):
                logger.info("Found product name from title tag")
    ...
    def _extract_images(self, soup: BeautifulSoup) -> List[str]:
        """Extract product images with enhanced filtering for duplicates and badges."""
        image_urls = set()
        
        # List of patterns to exclude (Prime badges, icons, etc.)
        exclude_patterns = [
        ...
        # Minimum dimensions to filter out small images
        MIN_WIDTH = 100
        MIN_HEIGHT = 100
        ...
        def is_valid_product_image(url: str) -> bool:
            ...
            for pattern in exclude_patterns:
                if pattern in url_lower:
                    logger.debug(f"Excluding image with pattern '{pattern}': {url[:100]}")
                    return False
            
            # Exclude common badge/icon paths
            if any(path in url_lower for path in [
                '/g/', '/x-locale/', '/playbutton/', '/kfs-assets/',
                '/coins/', '/sprites/', '/buttons/'
            ]):
                return False
        ...
        def extract_size_from_url(url: str) -> tuple:
            """Extract image dimensions from URL if present."""
            size_match = re.search(r'\._(AC_)?[A-Z]{2}(\d+)_', url)
            if size_match:
        ...
        def clean_amazon_url(url: str) -> str:
            """Clean and optimize Amazon image URL."""
            if not url:
                return url
            
            # Remove size constraints to get highest quality
            cleaned = re.sub(r'\._[A-Z0-9_,]+_\.', '.', url)
            
            # Remove tracking parameters
            cleaned = re.sub(r'(\?|&)(psc|ref|tag|qid|sr)=[^&]*', '', cleaned)
            
            # Convert webp to jpg when possible
            cleaned = re.sub(r'\.webp(\?|$)', '.jpg\\1', cleaned)
        ...
        if json_ld and "image" in json_ld:
            ...
            if isinstance(images, str):
                if is_valid_product_image(images):
                    image_urls.add(clean_amazon_url(images))
            elif isinstance(images, list):
                for img in images:
                    if is_valid_product_image(img):
                        image_urls.add(clean_amazon_url(img))
        
        # Strategy 2: Main product image container
        main_image_container = soup.select_one('#main-image-container')
        if main_image_container:
            img_elements = main_image_container.select('img[data-old-hires]')
            for img in img_elements:
                img_url = img.get('data-old-hires')
                if is_valid_product_image(img_url):
                    image_urls.add(clean_amazon_url(img_url))
        
        # Strategy 3: Dynamic image data (most reliable for Amazon)
        img_containers = soup.select('[data-a-dynamic-image]')
        for img_container in img_containers:
            dynamic_data = img_container.get('data-a-dynamic-image')
            if dynamic_data:
                try:
                    dynamic_images = json.loads(dynamic_data)
                    # Sort by size (largest first)
                    sorted_images = sorted(
                        dynamic_images.items(),
                        key=lambda x: x[1][0] * x[1][1] if len(x[1]) >= 2 else 0,
                        reverse=True
                    )
                    # Take only high-quality images
                    for img_url, dimensions in sorted_images[:10]:
                        if is_valid_product_image(img_url):
                            if len(dimensions) >= 2:
                    ...
                    logger.debug(f"Failed to parse dynamic image data: {e}")
        
        # Strategy 4: Image blocks (gallery images)
        image_blocks = soup.select('.image-block img, .imageBlock img, .imageThumbnail img')
        for img in image_blocks:
            for attr in ['data-old-hires', 'data-a-hires', 'src', 'data-src']:
                img_url = img.get(attr)
                if is_valid_product_image(img_url):
                    width, height = extract_size_from_url(img_url)
                    if width >= MIN_WIDTH and height >= MIN_HEIGHT:
                        image_urls.add(clean_amazon_url(img_url))
                        break
        
        # Strategy 5: Alternative image containers (but skip thumbnails)
        alt_selectors = [
            '#landingImage',
            '.a-dynamic-image:not(.a-thumbnail-image)',
            'img.a-dynamic-image[data-a-image-source]',
            '#imgBlkFront',
            '#ebooksImgBlkFront',
            '#main-image',
        ...
        for selector in alt_selectors:
            ...
            for img in images:
                for attr in ['data-old-hires', 'data-a-image-source', 'data-src', 'src']:
                    img_url = img.get(attr)
                    if is_valid_product_image(img_url):
                        # Skip small thumbnails based on URL patterns
                        if not re.search(r'\._(SX|SY|SS|CR)[0-9]{1,2}_', img_url):
                            image_urls.add(clean_amazon_url(img_url))
                            break
        
        # Strategy 6: Image gallery data
        gallery_data = soup.select('[data-a-image-name="landingImage"]')
        for elem in gallery_data:
            img_url = elem.get('data-a-image-source') or elem.get('src')
            if is_valid_product_image(img_url):
                image_urls.add(clean_amazon_url(img_url))
        ...
        for url in image_urls:
            # Create hash based on core filename
            filename_match = re.search(r'/([A-Z0-9]+)\.(jpg|jpeg|png)', url, re.IGNORECASE)
            if filename_match:
                file_hash = filename_match.group(1)
                if file_hash not in seen_hashes:
                    seen_hashes.add(file_hash)
                    unique_images.append(url)
            else:
                # Use normalized URL for comparison
                normalized = re.sub(r'\._(AC_)?[A-Z0-9_,]+_', '', url)
                normalized = re.sub(r'\.(jpg|jpeg|png).*$', '.\\1', normalized)
        ...
        # Log results
        if unique_images:
            logger.info(f"Found {len(unique_images)} unique product images after filtering")
        else:
            logger.warning("No valid product images found after filtering")
            # Last resort: try to get at least the main image
            main_img = soup.select_one('#landingImage, #imgBlkFront, .a-dynamic-image')
            if main_img:
                fallback_url = main_img.get('src') or main_img.get('data-src')
                if fallback_url and fallback_url.startswith('http'):
                    logger.info("Using fallback image")
                    unique_images.append(clean_amazon_url(fallback_url))
        
        # Return maximum 10 high-quality unique images
            ...
            if price_el:
                ...
                if price_text:
                    ...
                    if price_match:
                        ...
                        if canonical and ".ca/" in canonical.get("href", ""):
                            currency = "CAD"
        ...
        try:
            ...
            if json_data:
                ...
                product.description = product_info.get(
                    "shortDescription", "Product Description Not Found"
                )
...
def process_airtable_records():
    ...
    if not Api or AIRTABLE_API_KEY == "YOUR_AIRTABLE_API_KEY_HERE":
        logger.error(
            "❌ Airtable API is not configured. Please set AIRTABLE_API_KEY and BASE_ID."
        )
        return
    ...
    try:
        ...
        logger.info(
            f"✅ Connected to Airtable base '{AIRTABLE_BASE_ID}', table '{AIRTABLE_TABLE_NAME}'"
        )
    except Exception as e:
    ...
    for i, record in enumerate(records_to_process, 1):
        ...
        if product_code and str(product_code).strip():
            logger.info(
                f"✓ Product code '{product_code}' found in '4more-Product-Code' field for record {record_id} - skipping scraping"
            )
            try:
                table.update(
                    record_id,
                ...
                logger.info(
                    f"✅ Record {record_id} status updated to 'Linked to Catalogue'"
                )
            except Exception as e:
        ...
        if not product_url or not product_url.strip():
            table.update(
                record_id,
                {
                    ...
                    FIELD_MAPPINGS["scraping_status"]: "Skipped (No URL)",
                },
            )
            continue
        ...
        if not scraper:
            ...
            table.update(
                ...
                {
                    FIELD_MAPPINGS["status"]: "Needs Attention",
                    FIELD_MAPPINGS["scraping_status"]: "Skipped (Unsupported Website)",
                },
            )
            continue
